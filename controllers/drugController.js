...
export const createDrug = async (req, res) => {
  try {
    ...
    const drug = new Drug({ name, category, manufacturer, quantity, price, expiry });
    await drug.save();

    res.status(201).json(drug);
  } catch (err) {
    res.status(500).json({ message: "Error creating drug", error: err.message });
  }
};
...
export const updateDrug = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error updating drug", error: err.message });
  }
};
